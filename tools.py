...
def fetch_crawler_results(apify: ApifyWrapper, urls: List[str]) -> List[str]:
    loader = apify.call_actor(
        ...
        run_input={"startUrls": [{"url": url} for url in urls], "maxCrawlDepth": 1, "maxCrawlPages": 10},
...
def search_tool(query: str, top_n: int=3) -> SearchResultsResponse:
    ...
    loader = apify.call_actor(
        actor_id="apify/website-content-crawler",
        run_input={"startUrls": [{"url": url} for url in link_to_result.keys()], "maxCrawlDepth": 0, "maxCrawlPages": 10, "crawlerType": "cheerio"},
        dataset_mapping_function=my_dataset_mapping_function
    )
    documents = loader.load()
