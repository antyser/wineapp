    ...
    if st.button("Find Wine Information") or wine_query:
        try:
            ...
            message = build_human_message(
                text=(
                    get_instruction(wine_query, image_bytes is not None)
                    if first_message
                    else wine_query
                ),
...
def main_cmd(command: str, image_file: Optional[str] = None):
    ...
    # Use the build_human_message function to create the message
    message = build_human_message(
        text=get_instruction(command), image_bytes=image_bytes
    )

    # Invoke the agent with the message
    config = {"configurable": {"thread_id": "1"}}
    events = app.stream({"messages": [message]}, config, stream_mode="values")
    for event in events:
